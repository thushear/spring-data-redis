...
public interface ReactiveHashCommands {
	...
	default Flux<ByteBuffer> hKeys(ByteBuffer key) {
		...
		return hKeys(Mono.just(new KeyCommand(key))).next().flatMap(CommandResponse::getOutput);
	}
	...
	default Flux<ByteBuffer> hVals(ByteBuffer key) {
		...
		return hVals(Mono.just(new KeyCommand(key))).next().flatMap(CommandResponse::getOutput);
	}
	...
	default Flux<Map.Entry<ByteBuffer, ByteBuffer>> hGetAll(ByteBuffer key) {
		...
		return hGetAll(Mono.just(new KeyCommand(key))).next().flatMap(CommandResponse::getOutput);
	}
