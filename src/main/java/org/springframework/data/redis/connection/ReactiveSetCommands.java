...
public interface ReactiveSetCommands {
	...
	default Flux<ByteBuffer> sInter(Collection<ByteBuffer> keys) {
		...
		return sInter(Mono.just(SInterCommand.keys(keys))).next().flatMap(v -> v.getOutput());
	}
	...
	default Flux<ByteBuffer> sUnion(Collection<ByteBuffer> keys) {
		...
		return sUnion(Mono.just(SUnionCommand.keys(keys))).next().flatMap(CommandResponse::getOutput);
	}
	...
	default Flux<ByteBuffer> sDiff(Collection<ByteBuffer> keys) {
		...
		return sDiff(Mono.just(SDiffCommand.keys(keys))).next().flatMap(CommandResponse::getOutput);
	}
	...
	default Flux<ByteBuffer> sMembers(ByteBuffer key) {
		...
		return sMembers(Mono.just(new KeyCommand(key))).next().flatMap(CommandResponse::getOutput);
	}
